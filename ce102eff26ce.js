const LANG_LABELS={'pseudo':'Pseudocode','python':'Python','typescript':'TypeScript','clang':'C','cpp':'C++','javascript':'JavaScript','java':'Java','go':'Go','rust':'Rust','html':'HTML','css':'CSS'};const ICON_SPLIT=`<svg viewBox="0 0 24 24"><path d="M4 4h16v6H4V4zm0 10h16v6H4v-6z"/></svg>`;const ICON_COPY=`<svg viewBox="0 0 24 24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>`;const ICON_CHECK=`<svg viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>`;function runAlgoInit(){document.querySelectorAll('object[id^="kms://info.algo"]').forEach(wrapper=>{if(wrapper.getAttribute('data-init')==='true')return;wrapper.setAttribute('data-init','true');initAlgoWrapper(wrapper)})}
runAlgoInit();document.addEventListener('DOMContentLoaded',runAlgoInit);const observer=new MutationObserver((mutations)=>{runAlgoInit()});observer.observe(document.body,{childList:!0,subtree:!0});function initAlgoWrapper(wrapper){const rawObjects=Array.from(wrapper.querySelectorAll('object'));if(rawObjects.length===0)return;const availableLangs=rawObjects.map(obj=>{const langKey=obj.getAttribute('data-lang');return{id:langKey,name:LANG_LABELS[langKey]||langKey.charAt(0).toUpperCase()+langKey.slice(1)}});const defaultLang=availableLangs[0];const controls=document.createElement('div');controls.className='algo-controls';const primaryContainer=document.createElement('div');primaryContainer.className='algo-container primary';const secondaryContainer=document.createElement('div');secondaryContainer.className='algo-container secondary';controls.innerHTML=`
